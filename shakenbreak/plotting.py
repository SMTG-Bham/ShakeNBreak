...
def _format_defect_name(
    ...
    defect_name = None
    dummy_h = Element("H")
    # Vacancy: v/vac/va/vacancy, underscore and element symbol
    if any(
            [
                f"{substring}_" in defect_species.lower()  # defect shorthand plus underscore
                and any((dummy_h.is_valid_symbol(subsubstring[:2]) or dummy_h.is_valid_symbol(
                    subsubstring[-2:]) or dummy_h.is_valid_symbol(subsubstring[:1]) or
                                           dummy_h.is_valid_symbol(
                    subsubstring[-1:]) for subsubstring in defect_species.split("_") if
                        subsubstring != substring))  #
                # Element
                # symbol right before or after an underscore
                for substring in (
                    "vac",
                    "v",
                    "va",
    ...
    ): # TODO: use this function whenever we test the defect name (search for 'subsstring'
        # usages and "vac" usages)
        # vacancy... defect_name

    # else check if possibly different-format vacancy: (vCd/vaCd/vacCd/vacancyCd):
    elif any( ## I feel like this could give a lot of false positives; Only test 2-char elements
            # after "v"?
            [
                f"{substring}" in defect_species.lower()  # defect shorthand
                and any((dummy_h.is_valid_symbol(subsubstring[:2]) or dummy_h.is_valid_symbol(
                    subsubstring[:1]) for subsubstring in defect_species.split(substring) if
                        subsubstring != substring))  # Element symbol right after v/va/vac/vacancy
                for substring in (
                    "vac",
                    "v",
                    "va",
        ...
        # vacancy... poss_defect_name

    # Interstitial: int/interstitial (case-insensitive) or i_ (case-sensitive), underscore and
    # element for interstitial
    if any(
            [
                (f"{substring}_" in defect_species.lower()  # defect shorthand plus underscore
                # Element symbol right before or after an underscore
                for substring in (
                    "int",
                    "interstitial",
            ) or defect_species.startswith("i")) and any((dummy_h.is_valid_symbol(subsubstring[:2])
                                                        or
                                dummy_h.is_valid_symbol(
                    subsubstring[-2:]) or dummy_h.is_valid_symbol(subsubstring[:1]) or
                         dummy_h.is_valid_symbol(
                             subsubstring[-1:]) for subsubstring in defect_species.split("_")))  # TODO: Test each of these!
            ]
    ):
        # interstitial... defect_name
        # if defect_name is not None: whoops

    # or (new pymatgen naming) Element + _i (i.e. Cd_i)
    if any(
            [
                f"_i" in defect_species  # defect shorthand plus underscore
                and any(dummy_h.is_valid_symbol(defect_species.split("_i")[0][-2:]) or dummy_h.is_valid_symbol(
                    defect_species.split("_i")[0][-1:]))  # Element symbol right before or after an underscor
            ]
    ):
        # interstitial... defect_name
        # if defect_name is not None: whoops

    # else check if possibly different-format interstitial: (iCd/intCd/interstitialCd):
    ## I feel like this could give a lot of false positives? Only test 2-char elements after "i"?
    elif any(
        [
            f"{substring}" in defect_species.lower()  # defect shorthand
            and any((dummy_h.is_valid_symbol(subsubstring[:2]) or dummy_h.is_valid_symbol(
                subsubstring[:1]) for subsubstring in defect_species.split(substring, 1)[1] if
                     subsubstring != substring))  # Element symbol right after v/va/vac/vacancy
            for substring in (
            "i",
            "int",
    ...
    # as/antisite/sub/substitution/none and element, (underscore), element for substitution
    if any(
            [
                f"{substring}_" in defect_species.lower()  # defect shorthand plus underscore
                for substring in (
        ...
        # subs_element is valid symbol just before/after an underscore
        poss_subs_element = [
            subsubstring
            for subsubstring in defect_species.split("_")
            if dummy_h.is_valid_symbol(subsubstring[:2])
            or dummy_h.is_valid_symbol(subsubstring[-2:])
        ]
        # site is valid symbol just before/after an underscore, after subs_element and not the
        # same as subs_element
        poss_site = [
            subsubstring
            for subsubstring in defect_species.split("_")
            if dummy_h.is_valid_symbol(subsubstring[:2])
            or dummy_h.is_valid_symbol(subsubstring[-2:])
            and subsubstring != poss_subs_element[0]
        ]
        if not poss_subs_element or not poss_site:
            # doesn't match expected format, test for any (two-character) Element followed by a (
            # two-character) Element as last-ditch check
            two_char_substrings = [
                defect_species[i : i + 2]
                for i in range(len(defect_species) - 1)
                if dummy_h.is_valid_symbol(defect_species[i : i + 2])
            ]
            if len(two_char_substrings) == 2:
                poss_subs_element = [two_char_substrings[0]]
                poss_site = [two_char_substrings[1]]

        elif len(poss_subs_element) > 1 or len(poss_site) > 1 and defect_name is None:
            # more than one match but no previous vacancy/interstitial match so assume first two

    elif defect_name is None:  # check for antisite/substitution as Element_Element or
        # ElementElement or Element_on_Element or similar
        # subs_element is valid symbol just before/after an underscore
        poss_subs_element = [
            subsubstring
            for subsubstring in defect_species.split("_")
    ...
    # still no matches, check for antisites/substitutions without substring
    # or Element_Element or Element_on_Element (or ElementElement where both are two-letter
    # elements and no match so far)


    # and may have site number included (must be surrounded by underscores if so)

    # TODO: Raise this error in functions that use _validate_defect_name() but get false when
    #  should be true
    #  raise ValueError(
    ...
    #     "the config file."
    # )
    # TODO: Add/update warning in cases where we loop over looking for dirs but don't find
    #  any matching directories; specify what matches we're looking for

    # When format_defect_name is used, should use try except where if not recognised and
    # formatted, we ignore (plot without defect naming) and raise warning saying what format we
    ...
    if defect_name is None:
        raise ValueError("Defect name not recognised. Please check spelling. Got... expected...")
