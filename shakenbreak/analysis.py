...
def _format_distorion_names(
    distortion_label: str,
):
    """
    Formats the distortion filename to the names used internally and for analysis. (i.e. 'Bond_Distortion_-50.0%' -> -0.5)
    Args:
        distortion_label (:obj:`str`):
            distortion label used for file names.
    Returns:
        distortion (:obj:`float` or :obj:`float`):
            distortion factor (e.g. -0.6, 0.0, +0.6) or string (e.g. "Unperturbed" or "rattled")
    """
    if distortion_label == 'Unperturbed' or distortion_label == 'rattled':
        distortion = distortion_label
    elif distortion_label.startswith('Bond_Distortion') and distortion_label.endswith('%'):
        distortion = float(distortion_label.split("Bond_Distortion_")[-1].split("%")[0]) / 100
    elif distortion_label.startswith('Bond_Distortion') and ('_from_' in distortion_label): # distortions from other charge state of the defect
...
def analyse_structure(
    ...
    if not os.path.exists(f"{output_path}/distortion_metadata.json"):
        raise FileNotFoundError(f"No `distortion_metadata.json` file found in {output_path}. Use function `analyse_defect_site` instead.")

    distortion_metadata = _read_distortion_metadata(output_path=output_path) # Read site from distortion_metadata.json
...
# TODO: Modify `get_structures` to enable user to decide if parsing all subdirectories or only standard BDM, or champion, or both?
def get_structures(
    defect_species: str,
    output_path: str = ".",
    ...
    Args:
        ...
        bond_distortions (:obj:`list`, optional):
            List of bond distortions whose structures will be analysed (i.e. [-0.2, 0.0, 0.5]). By deafult, all distortions present in subdirectories
            are considered.
            (Default: None)
    Returns:
    ...
    if not bond_distortions: # if the user didnt specify any set of distortions, loop over subdirectories
        ...
        for distortion_subdirectory in distortion_subdirectories:
            ...
            if distortion != "Label_not_recognized": # If the subdirectory name is recognised
                try:
                    defect_structures_dict[distortion] = grab_contcar(f"{output_path}/{defect_species}/{distortion_subdirectory}/CONTCAR")
                except:
                    warnings.warn(
...
# TODO: Update `get_energies` to parse champion runs with new naming (from_<charge_state>)
def get_energies(
    defect_species: str,
    output_path: str = '.',
