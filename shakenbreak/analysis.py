...
def _site_magnetizations(
    ...
    Args:
        ...
        threshold (float, optional):
            Magnetization threshold to print site. Defaults to 0.1 e-.
    Returns:
    ...
    for index, element in enumerate(mag):
        ...
        if np.abs(total_mag) > threshold:
            ...
            significant_magnetizations[
                ...
                }  # dont include total site magnetization again
            )
...
def get_site_magnetizations(
    ...
    for distortion in distortions:
        dist_label = _get_distortion_filename(
            distortion
        )  # get filename (e.g. Bond_Distortion_50.0%)
        structure = grab_contcar(f"{output_path}/{defect}/{dist_label}/CONTCAR")
        assert os.path.exists(
            f"{output_path}/{defect}/{dist_label}/OUTCAR"
        ), f"OUTCAR file not found in path {output_path}/{defect}/{dist_label}/OUTCAR"
        outcar = Outcar(f"{output_path}/{defect}/{dist_label}/OUTCAR")
        print(
            f"Analysing distortion {distortion}. Total magnetization:",
            round(outcar.total_mag, 3),
        )
        df = _site_magnetizations(
            outcar=outcar, structure=structure, threshold=threshold
        )
        if not df.empty:
